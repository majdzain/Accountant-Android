...
public class CustomSearchRBondsAdapter extends ArrayAdapter<RBondListChildItem> {

    Context context;
    LayoutInflater inflater;
    private List<RBondListChildItem> bondsList;
    private ArrayList<RBondListChildItem> filteredRBondsList;
    String filteredText;

    public CustomSearchRBondsAdapter(Context context, int resId, ArrayList<RBondListChildItem> bonds) {
        super(context, resId);
        this.bondsList = bonds;
        this.filteredRBondsList = bonds;
        this.context = context;
        inflater = LayoutInflater.from(context);
        this.filteredRBondsList = new ArrayList<RBondListChildItem>();
        this.filteredRBondsList.addAll(bondsList);
        bondsList.clear();
    }
    ...
    @Override
    public RBondListChildItem getItem(int position) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        CustomSearchRBondsAdapter.ViewHolder holder = null;
        RBondListChildItem bond = getItem(position);

        LayoutInflater mInflater = (LayoutInflater) context.getSystemService(Activity.LAYOUT_INFLATER_SERVICE);
        if (convertView == null) {
            convertView = mInflater.inflate(R.layout.list_bonds_item, null);
            holder = new CustomSearchRBondsAdapter.ViewHolder();
            ...
            holder = (CustomSearchRBondsAdapter.ViewHolder) convertView.getTag();
        holder.bondLinear.setBackground(context.getResources().getDrawable(R.drawable.list_r_bond_item_selector));
        String name = bond.getCreditor();
    ...
    public void filter(String charText) {
        ...
        if (charText.length() == 0) {
            ...
            for (RBondListChildItem bond : filteredRBondsList) {
                if ((bond.getCreditor() + bond.getTime() + bond.getDate()+ bond.getPhone()).toLowerCase(Locale.getDefault()).contains(charText)) {
                    bondsList.add(bond);
                }
            }
        }
